...
class BasicAuxRestorer(BaseModel):
    ...
    def forward(self, lq, aux=None, gt=None, test_mode=False, **kwargs):
        """Forward function.
        ...
        return self.forward_train(lq, aux, gt)

    def forward_train(self, lq, aux, gt):
        ...
        """
        losses = dict()
        output, aux_lr = self.generator(lq)
        loss_pix = self.pixel_loss(output, gt)
        loss_aux = self.pixel_loss(aux_lr, aux)
        losses['loss_pix'] = loss_pix + loss_aux
        outputs = dict(
            losses=losses,
