...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        #     add_identity=False,
        #     init_cfg=None)
        self.mlp = Mlp(in_features=embed_dims, hidden_features=mlp_hidden_dim, act_layer=nn.GELU, drop=0.)        
...
class SwinAuxIR(nn.Module):
    ...
    def __init__(self,
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.conv_before_upsample = nn.Sequential(
                ...
                nn.LeakyReLU(inplace=True))            
            self.upsample = Upsample(upscale_factor, num_feat)
            self.conv_last_up = nn.Conv2d(num_feat, num_out_ch, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = self.conv_before_upsample(x)
            before_ = x.clone()
            before_ = self.conv_aux(before_)
            x = self.conv_after_aux(before_)
            x = self.conv_last(self.upsample(x))
        elif self.upsampler == 'pixelshuffledirect':
            # for lightweight SR
            x = self.conv_first(x)
